...
class OpenPosePredictor(ABC):
    ...
    def _init__model_infor(self) -> None:
        """
        > This function initializes the model information and will be implemented in subclass corresponding to the model
        """
    ...
    def _create_model(self, prototxt_path: str, weight_path: str) -> cv2.dnn.Net:
        ...
        Args:
          prototxt_path (str): The path to the prototxt file.
            Download from: https://github.com/CMU-Perceptual-Computing-Lab/openpose/tree/master/models/pose
          weight_path (str): The path to the Caffe model file.
            Download guide: https://github.com/CMU-Perceptual-Computing-Lab/openpose/blob/master/models/getModels.sh
    ...
    def _prepare_input(self, img: np.array) -> None:
        """
        It takes an image, resizes it to the input size of the network, and then sets the image as the input
        to the network
        
        Args:
          img (np.array): the input image
        """
        inp_blob = cv2.dnn.blobFromImage(img, 
                                        1.0 / 255, 
                                        self.in_size,
                                        (0, 0, 0), 
                                        swapRB=False, 
    ...
    def _parse_keypoints(self, net_ouput: cv2.Mat, img_size: tuple) -> list:
        ...
        Args:
          net_ouput (cv2.Mat): the output of the neural network (4D matrix).
            ...
            - 2nd dim: index of a keypoint.  Ex COCO:  18 keypoint confidence maps + 1 background + 19*2 Part Affinity Maps
    ...
    def visualize(self, keypoints: list, img, put_text: bool = False):
        """
        It takes a list of keypoints and an image, and draws the keypoints and skeleton on the image.
        
        Args:
          keypoints (list): list of keypoints
          img: The image to draw the skeleton on
          put_text (bool): If True, the keypoints will be labeled with their index. Defaults to False
    ...
    def _visualize_points(self, keypoints: list, img, put_text: bool = False):
        ...
        for i in range(len(self.BODY_PARTS)):
            if keypoints[i]:
                ...
                if put_text:
                    cv2.putText(img, "{}".format(i), (int(x), int(y)),
...
def add_keypoints_to_json(self, img: np.array, pose_predictor: OpenPosePredictor):
    ...
    """
    keypoints = [p if p else (0, 0, 0) for p in pose_predictor.predict(img)] # replace None with [0, 0, 0]
